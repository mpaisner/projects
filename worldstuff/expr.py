...
class Expression:
	...
	def copy(self):
		return Expression(self.kind, self.args, self.beliefLevel, self.duration)
	...
	def replace_to_match(self, atom, match, types = [FORALL, EXISTS]):
		replacedVars = {}
		replacing = True
		for i in range(len(atom.args)):
			if match.args[i] == atom.args[i]:
				continue
			elif match.args[i].kind in types:
				if atom.args[i].kind == ITEM:
					replacedVars[match.args[i].id] = atom.args[i].inst
				elif atom.args[i].kind == match.args[i].kind:
					replacedVars[match.args[i].id] = atom.args[i].id
				else:
					replacing = False
					break
			else:
		...
		if replacing:
			expr = self
			for var, val in replacedVars.items():
	...
	def same_belief(self, other):
		...
		for i in range(len(self.args)):
			if not self.args[i].same_belief(other.args[i]):
				return False
		return self.beliefLevel * other.beliefLevel > 0 #valence of belief is the same
	
	def opposite_belief(self, val):
		if type(self) != type(other) or self.kind != other.kind or len(self.args) != len(other.args):
			return False
		for i in range(len(self.args)):
			if not self.args[i].same_belief(other.args[i]):
				return False
		return self.beliefLevel * other.beliefLevel < 0 #valence of belief is opposite
...
class Atom(Expression):
	...
	def copy(self):
		return Atom(self.predicate, self.args, self.beliefLevel, self.duration)

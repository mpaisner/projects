...
class Action:
	...
	def __init__(self, args, operator, preconds, prePos, results, postPos):
		self.args = args
...
class Operator:
	...
	def instantiate(self, args):
		...
		return Action([objdict[self.objnames[i]] for i in range(len(self.objnames))], self, preconditions, self.prePos, results, self.postPos)
...
class World:
	...
	def get_possible_actions(self, op = None, args = []):
		actions = []
		if not op:
			for operator in self.operators.values():
				actions += self.get_possible_actions(operator, args)
			return actions
		else:
			#print "trying op:", op.name
			if len(args) == len(op.objnames):
				action = op.instantiate(args)
				...
				for obj in self.objects.values():
					actions += self.get_possible_actions(op, args + [obj])
				return actions
